...
const ServicoProfissionaisList = ({ servicoId }: Props) => {
  ...
  const fetchProfissionais = useCallback(async () => {
    ...
    try {
      setLoading(true);
      // CORREÇÃO: URL alterada de /servicos para /services
      const res = await api.get(`/services/${servicoId}/profissionais`);
  ...
  const handleRemove = async (profId: string) => {
    if (!confirm("Desvincular profissional deste serviço?")) return;
    try {
      // CORREÇÃO: URL alterada de /servicos para /services

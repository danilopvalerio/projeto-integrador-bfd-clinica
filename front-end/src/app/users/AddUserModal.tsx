...
import UserGeneralForm, { UserFormData } from "./UserGeneralForm"; // Importando a interface
import { CreateUserPayload, UserType } from "./types";
...
const AddUserModal = ({ onClose, onSuccess }: Props) => {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");

  // Usamos a interface exportada do form para garantir compatibilidade
  ...
  // FUNÇÃO DE MUDANÇA GENÉRICA E ESTRITA
      ...
      >
        <div className="modal-content border-0 shadow rounded-4">
          <div className="modal-header border-bottom-0 p-4 pb-0">
            <h5 className="modal-title fw-bold text-secondary">Novo Usuário</h5>
            <button
              type="button"
              className="btn-close"
              onClick={onClose}
          ...
          <div className="modal-body p-4 pt-2">
            ...
            <form onSubmit={handleSubmit}>
              <UserGeneralForm
                data={formData}
                onChange={handleChange} // Passamos a função tipada
                mode="create"
                disabled={loading}
              />
